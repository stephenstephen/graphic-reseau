...
define([
    ...
    $.widget('mageplaza.layer', $.mage.accordion, {
        ...
        initObserve: function () {
            ...
            const cloakClassName = ".quatrecentdix";
            var self = this;
            ...
            var currentElements = this.element.find(`.filter-current a, .filter-actions a,.filter-current ${cloakClassName}, .filter-actions ${cloakClassName}`);
            currentElements.each(function (index) {
                var el = $(this),
                    link = self.checkUrl(el.prop('href'));

                if (!link) {
                    link = self.checkUrl(window.atob(el.attr("data-atc")));
            ...
            var optionElements = this.element.find(`.filter-options a,.filter-options ${cloakClassName}`);
            optionElements.each(function (index) {
                ...
                if (!link) {
                    link = self.checkUrl(window.atob(el.attr("data-atc")));
                    link = link.replaceAll("amp;", ""); // fix redirect loop !
                }

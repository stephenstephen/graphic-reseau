...
class Info extends \Magento\Payment\Block\Info
{
    ...
    public function getResultDescHtml()
    {
        ...
        if (! is_array($allResults) || empty($allResults)) {
            ...
            $restError = $this->getInfo()->getAdditionalInformation(\Lyranetwork\Systempay\Helper\Payment::REST_ERROR_MESSAGE);
            if ($restError) {
                $labels[] = $restError;
                unset($keys[0]);
            }
    ...
    public function getMultiPaymentDetailsHtml($backend = true)
    {
        ...
        $userChoice = $this->getInfo()->getAdditionalInformation(\Lyranetwork\Systempay\Helper\Payment::BRAND_USER_CHOICE);

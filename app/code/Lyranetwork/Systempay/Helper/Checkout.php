    ...
    */
    ...
    public function checkCustormers($scope, $scopeId)
    {
        ...
        foreach ($collection as $customer) {
            if (! preg_match(self::CUST_ID_REGEX, $customer->getId())) {
                ...
                $msg = '';
                $msg .= __(
                    'Customer ID &laquo; %1 &raquo; does not match Systempay specifications.',
                    $customer->getId()
                )->render() . ' ';
    ...
    private function checkOrderId($orderId)
    {
        if (! preg_match(self::ORDER_ID_REGEX, $orderId)) {
            ...
            $msg = '';
            $msg .= __(
                'The next order ID  &laquo; %1 &raquo; does not match Systempay specifications.',
                $orderId
            )->render() . ' ';
    ...
    public function checkProducts($scope, $scopeId)
    {
        ...
        foreach ($collection as $product) {
            if (! preg_match(self::PRODUCT_REF_REGEX, $product->getId())) {
                ...
                $msg = '';
                $msg .= __(
                    'Product reference &laquo; %1 &raquo; does not match Systempay specifications.',
                    $product->getId()
                )->render() . ' ';
        ...
        if (is_array($categoryMapping) && ! empty($categoryMapping) && is_array($categoryIds) && ! empty($categoryIds)) {
            foreach ($categoryMapping as $category) {
                if (in_array($category['code'], $categoryIds)) {
                    return $category['systempay_category'];
                }
            }
        }
    ...
    public function setCartData($order, &$systempayRequest)
    {
        ...
        // Used currency.
        $currency = SystempayApi::findCurrencyByNumCode($systempayRequest->get('currency'));
        ...
        $systempayRequest->set('totalamount_vat', $taxAmount);
    }

    public function setOneyData($order, &$systempayRequest)
    {
        ...
        if ($order->getIsVirtual() || ! $order->getShippingMethod()) { // There is no shipping method.
            ...
            switch ($shippingMethod['type']) {
                ...
                case 'RECLAIM_IN_STATION':
                    ...
                    $systempayRequest->set('ship_to_street2', $street2);
                    $systempayRequest->set('ship_to_state', null);

                    $method = $carrierName . ' ' . $address;
                    $systempayRequest->set('ship_to_delivery_company_name', $method);
                    break;
                ...
                default:
                    ...
                    $systempayRequest->set('ship_to_street', $address);
                    $systempayRequest->set('ship_to_street2', null); // Not sent to Oney.

                    $systempayRequest->set('ship_to_delivery_company_name', $carrierName);
                    break;
            }
            ...
            $systempayRequest->set('ship_to_type', empty($shippingMethod['type']) ? null : $shippingMethod['type']);
            $systempayRequest->set('ship_to_speed', empty($shippingMethod['speed']) ? null : $shippingMethod['speed']);
    ...
    {
        if ($this->cookieManager->getCookie('systempay_oney_error')) {

 ...
 */
namespace Lyranetwork\Systempay\Helper;
...
class Payment
{
    ...
    const IDENTIFIER = 'systempay_identifier';
    const SEPA_IDENTIFIER = 'systempay_identifier';

    // Key to save choosen multi option.
    const MULTI_OPTION = 'systempay_multi_option';

    // Key to save choosen Choozeo option.
    const CHOOZEO_OPTION = 'systempay_choozeo_option';

    // Key to save choosen Oney option.
    const ONEY_OPTION = 'systempay_oney_option';

    // Key to save choosen Full CB option.
    const FULLCB_OPTION = 'systempay_fullcb_option';

    // Key to save choosen FranFinance option.
    const FRANFINANCE_OPTION = 'systempay_franfinance_option';

    // Key to save choosen other payment means.
    const OTHER_OPTION = 'systempay_other_option';

    // Key to save risk control results.
    const RISK_CONTROL = 'systempay_risk_control';

    // Key to save risk assessment results.
    const RISK_ASSESSMENT = 'systempay_risk_assessment';

    // Key to save payment results.
    const ALL_RESULTS = 'systempay_all_results';

    // Key to save Rest Api error message.
    const REST_ERROR_MESSAGE = 'systempay_rest_error';

    const TRANS_UUID = 'systempay_trans_uuid';

    const BRAND_USER_CHOICE = 'systempay_brand_user_choice';
     ...
     * @param \Lyranetwork\Systempay\Model\Api\SystempayResponse $response
     */
    public function registerOrder(
        \Magento\Sales\Model\Order $order,
        \Lyranetwork\Systempay\Model\Api\SystempayResponse $response
    ) {
        ...
        if ($method instanceof \Lyranetwork\Systempay\Model\Method\Sepa)  {
    ...
    public function nextOrderState(
        ...
        if ($response->isToValidatePayment()) {
            ...
            if ($this->isSepa($response)) {
                ...
                $newStatus = 'systempay_pending_transfer';
            } else {
    ...
    public function updatePaymentInfo(\Magento\Sales\Model\Order $order, \Lyranetwork\Systempay\Model\Api\SystempayResponse $response)
    {
        ...
        $order->getPayment()
            ...
            ->setAdditionalInformation(\Lyranetwork\Systempay\Helper\Payment::ALL_RESULTS, serialize($response->getAllResults()))
            ->setAdditionalInformation(\Lyranetwork\Systempay\Helper\Payment::TRANS_UUID, $response->get('trans_uuid'));
        ...
        if ($restErrorMsg) {
            ...
            $order->getPayment()->setAdditionalInformation(\Lyranetwork\Systempay\Helper\Payment::REST_ERROR_MESSAGE, $restErrorMsg);
        }
        ...
        if ($response->get('brand_management')) {
            ...
            $order->getPayment()->setAdditionalInformation(\Lyranetwork\Systempay\Helper\Payment::BRAND_USER_CHOICE, $userChoice);
        }
        ...
        if ($response->get('card_brand') === 'MULTI') { // Multi brand.
            ...
            $currency = SystempayApi::findCurrencyByNumCode($response->get('currency'));
            ...
            $option = @unserialize($order->getPayment()->getAdditionalInformation(\Lyranetwork\Systempay\Helper\Payment::MULTI_OPTION));
            ...
            if ($isFirstInstallment && (stripos($order->getPayment()->getMethod(), 'systempay_multi') === 0)
                ...
                for ($i = 1; $i <= $count; $i++) {
                    ...
                    if (($rate = $response->get('change_rate')) && $response->get('effective_currency') &&
                        ...
                        // Effective amount.
                        $effectiveCurrency = SystempayApi::findCurrencyByNumCode($response->get('effective_currency'));
    ...
    public function saveIdentifier(
        ...
        if ($response->get('identifier') && (
            ...
            $customerData->setCustomAttribute('systempay_identifier', $response->get('identifier'));

            // Mask card number and save it to customer entity.
            $customerData->setCustomAttribute('systempay_masked_pan', $this->maskPan($response));

            try {
                $customer->updateData($customerData);

                $customerResource = $this->customerResourceFactory->create();
                $customerResource->saveAttribute($customer, 'systempay_identifier');
                $customerResource->saveAttribute($customer, 'systempay_masked_pan');
    ...
    public function saveSepaIdentifier(
        ...
        if ($response->get('identifier') && (
            ...
            $customerData->setCustomAttribute('systempay_sepa_identifier', $response->get('identifier'));

            // Mask IBAN and save it to customer entity.
            $customerData->setCustomAttribute('systempay_sepa_iban_bic', $this->maskPan($response));

            try {
                $customer->updateData($customerData);

                $customerResource = $this->customerResourceFactory->create();
                $customerResource->saveAttribute($customer, 'systempay_sepa_identifier');
                $customerResource->saveAttribute($customer, 'systempay_sepa_iban_bic');
    ...
    public function deleteIdentifier($customerId, $attribute, $maskedAttribute)
    {
        ...
        try {
            if ($this->restHelper->getPrivateKey()) {
                ...
                $client = new \Lyranetwork\Systempay\Model\Api\SystempayRest(
     ...
     * @return string
     */
    public function convertTransactionType($systempayType)
    {
        $type = false;

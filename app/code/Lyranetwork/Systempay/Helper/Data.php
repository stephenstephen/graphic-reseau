...
use Lyranetwork\Systempay\Block\Adminhtml\System\Config\Form\Field\Gift\AddedCards;
use Lyranetwork\Systempay\Model\Method\Gift;

class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
    const METHOD_STANDARD = 'systempay_standard';
    const METHOD_MULTI = 'systempay_multi';
    const METHOD_CHOOZEO = 'systempay_choozeo';
    const METHOD_SEPA = 'systempay_sepa';
    const METHOD_GIFT = 'systempay_gift';
    const METHOD_ONEY = 'systempay_oney';
    const METHOD_PAYPAL = 'systempay_paypal';
    const METHOD_FULLCB = 'systempay_fullcb';
    const METHOD_FRANFINANCE = 'systempay_franfinance';
    const METHOD_OTHER = 'systempay_other';
    ...
    /**
     * @var \Lyranetwork\Systempay\Model\Logger\Systempay
     */
    protected $logger;
    ...
    /**
     * @param \Lyranetwork\Systempay\Helper\Context $context
     * @param \Magento\Framework\ObjectManagerInterface $objectManager
     * @param \Magento\Store\Model\StoreManagerInterface $storeManager
     * @param \Magento\Framework\App\MaintenanceMode $maintenanceMode
     * @param \Magento\Config\Model\ResourceModel\Config $resourceConfig
     * @param \Magento\Framework\Filesystem $filesystem
     * @param \Lyranetwork\Systempay\Model\Logger\Systempay
    ...
    public function __construct(
        \Lyranetwork\Systempay\Helper\Context $context,
        \Magento\Framework\ObjectManagerInterface $objectManager,
        \Magento\Store\Model\StoreManagerInterface $storeManager,
        \Magento\Framework\App\MaintenanceMode $maintenanceMode,
        \Magento\Config\Model\ResourceModel\Config $resourceConfig,
        \Magento\Framework\Filesystem $filesystem,
        \Lyranetwork\Systempay\Model\Logger\Systempay $logger,
    ...
    public function getCommonConfigData($field, $storeId = null)
    {
        ...
        $value = $this->scopeConfig->getValue('systempay/general/' . $field, ScopeInterface::SCOPE_STORE, $storeId);
    ...
    public function getReturnUrl($storeId = null)
    {
        ...
        return $this->_getUrl('systempay/payment/response', $params);
    }
    ...
    {
        $filePath = $this->filesystem->getDirectoryRead(DirectoryList::MEDIA)->getAbsolutePath('systempay/images/' . $fileName);
    ...
    {
        // $path is as payment_[lang]/systempay/systempay_[group]/systempay_[sub_group]
        $parts = explode('/', $path);
    ...
    public function updateMultiPaymentModelConfig($options)
    {
        foreach ($options as $option) {
            $this->resourceConfig->saveConfig(
                'payment/systempay_multi_' . $option['count'] . 'x/model',
                \Lyranetwork\Systempay\Model\Method\Multix::class,
    ...
    public function getMultiPaymentModelConfig()
    {
        ...
        $select = $connection->select()
            ...
            ->where('path LIKE ?', 'payment/systempay\_multi\_%x/model');
    ...
    public function updateOtherPaymentModelConfig($options)
    {
        foreach ($options as $option) {
            $this->resourceConfig->saveConfig(
                'payment/systempay_other_' . $option['means'] . '/model',
                \Lyranetwork\Systempay\Model\Method\Other::class,
                ScopeConfigInterface::SCOPE_TYPE_DEFAULT,
                0
            );
        }
    ...
    public function getOtherPaymentModelConfig()
    {
        ...
        $select = $connection->select()
            ->from($this->resourceConfig->getMainTable())
            ->where('path LIKE ?', 'payment/systempay\_other\_%/model');
    ...
    public function getLogoImageSrc($name)
    {
        if ($this->isUploadFileImageExists($name)) {
            return $this->storeManager->getStore()->getBaseUrl(\Magento\Framework\UrlInterface::URL_TYPE_MEDIA) .
                'systempay/images/' . $name;
        } else {
            // Default logo from the installed plugin.
            $asset = $this->assetRepo->createAsset('Lyranetwork_Systempay::images/' . $name);

            if ($this->isPublishFileImageExists($asset->getRelativeSourceFilePath())) {
                return $this->getViewFileUrl('Lyranetwork_Systempay::images/' . $name);
            }
        }
    ...
    {
        $standardMethod = $this->getMethodInstance(\Lyranetwork\Systempay\Helper\Data::METHOD_STANDARD);
        $sepaMethod = $this->getMethodInstance(\Lyranetwork\Systempay\Helper\Data::METHOD_SEPA);

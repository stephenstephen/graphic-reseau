...
define(
    ...
    function ($, Component, url, storage, quote, setPaymentInformationAction, additionalValidators, errorProcessor, fullScreenLoader) {
        ...
        return Component.extend({
            defaults: {
                template: 'Lyranetwork_Systempay/payment/systempay-standard',
                systempayCcType:  window.checkoutConfig.payment.systempay_standard.availableCcTypes ?
                    window.checkoutConfig.payment.systempay_standard.availableCcTypes[0]['value'] : null,
            ...
            getData: function () {
                ...
                if (this.isOneClick()) {
                    data['additional_data']['systempay_standard_use_identifier'] = this.systempayUseIdentifier();
                }

                if (this.getEntryMode() == 2) { // Payment means selection on merchant site.
                    data['additional_data']['systempay_standard_cc_type'] = this.systempayCcType();
                }
            ...
            getRestFormToken: function () {
                return window.checkoutConfig.payment.systempay_standard.restFormToken || null;
            },

            getLanguage: function () {
                return window.checkoutConfig.payment.systempay_standard.language || null;
            },

            updatePaymentBlock: function (blockName) {
                $('.payment-method._active .payment-method-content .systempay-identifier li.systempay-block').hide();
                $('li.systempay-standard-' + blockName + '-block').show();
            },
                    ...
                    KR.setFormConfig({
                        ...
                        function(v) {
                            ...
                            KR.onFocus(function(e) {
                                $('#systempay_rest_form .kr-form-error').html('');
                            });
                            ...
                            KR.onError(function (e) {
                                ...
                                $('#systempay_rest_form .kr-form-error').html('<span style="color: red;"><span>' + msg + '</span></span>');
                            });
                        }
                    );
                });
            ...
            refreshToken: function () {
                ...
                storage.post(
                    url.build('systempay/payment_rest/token?form_key=' + $.mage.cookies.get('form_key'))
            ...
            initIframeEvents: function (elts) { // To be called after iframe loading.
                ...
                quote.paymentMethod.subscribe(function(method) {
                    if ((method.method === 'systempay_standard') || !me.iframeDisplayed) {
                        return;
                    }

                    var iframe = $('.payment-method .payment-method-content iframe.systempay-iframe');
                    if (iframe && iframe.length) {
                        var cancelUrl = me.getIframeLoaderUrl() + '?mode=cancel' + '&' + Date.now();
                        iframe.attr('src', cancelUrl);

                        $('.payment-method .payment-method-content .systempay-iframe').hide();
                        $('.payment-method .payment-method-content .systempay-form').show();
            ...
            afterPlaceOrder: function () {
                ...
                if (me.getEntryMode() == 3) { // Iframe mode.
                    ...
                    $('.payment-method._active .payment-method-content .systempay-form').hide();
                    $('.payment-method._active .payment-method-content .systempay-iframe').show();

                    var iframe = $('.payment-method._active .payment-method-content iframe.systempay-iframe');
                    if (iframe && iframe.length) {

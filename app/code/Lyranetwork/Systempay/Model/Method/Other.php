...
class Other extends Systempay
{
    protected $_code = \Lyranetwork\Systempay\Helper\Data::METHOD_OTHER;
    protected $_formBlockType = \Lyranetwork\Systempay\Block\Payment\Form\Other::class;
    ...
    protected function setExtraFields($order)
    {
        ...
        $option = @unserialize($info->getAdditionalInformation(\Lyranetwork\Systempay\Helper\Payment::OTHER_OPTION));

        // Check if capture_delay and validation_mode are overriden.
        if (is_numeric($option['capture_delay'])) {
            $this->systempayRequest->set('capture_delay', $option['capture_delay']);
        }

        if ($option['validation_mode'] !== '-1') {
            $this->systempayRequest->set('validation_mode', $option['validation_mode']);
        }

        // Add cart data.
        if ($option['cart_data'] === '1') {
            $this->checkoutHelper->setCartData($order, $this->systempayRequest, true);
        }
    }
    ...
    public function assignData(\Magento\Framework\DataObject $data)
    {
        ...
        $option = $this->_getMeans($systempayData->getData('systempay_other_option'));

        $info->setCcType($option['means'])
            ->setAdditionalInformation(\Lyranetwork\Systempay\Helper\Payment::OTHER_OPTION, serialize($option));
        return $this;
    }
    ...
    */
    ...
    {
        $supportedCards = \Lyranetwork\Systempay\Model\Api\SystempayApi::getSupportedCardTypes();

...
class Standard extends Systempay
{
    ...
    protected function setExtraFields($order)
    {
        ...
        if ($this->isIframeMode()) {
            ...
            $this->systempayRequest->set('theme_config', $this->systempayRequest->get('theme_config') . '3DS_LOGOS=false;');
        ...
        if ($this->isOneClickActive() && $order->getCustomerId()) {
            ...
            if ($customer->getCustomAttribute('systempay_identifier') && $this->customerSession->getValidAlias()) {
                // Customer has an identifier.
                $this->systempayRequest->set('identifier', $customer->getCustomAttribute('systempay_identifier')->getValue());

                if (! $info->getAdditionalInformation(\Lyranetwork\Systempay\Helper\Payment::IDENTIFIER)) {
    ...
    public function isOneclickAvailable()
    {
        ...
        if (! $customer || ! ($identifier = $customer->getCustomAttribute('systempay_identifier'))) {
            return false;
        }
    ...
    public function getRestApiFormToken()
    {
        ...
        // Currency.
        $currency = \Lyranetwork\Systempay\Model\Api\SystempayApi::findCurrencyByAlphaCode($quote->getOrderCurrencyCode());
        if ($currency == null) {
            // If currency is not supported, use base currency.
            $currency = \Lyranetwork\Systempay\Model\Api\SystempayApi::findCurrencyByAlphaCode($quote->getBaseCurrencyCode());

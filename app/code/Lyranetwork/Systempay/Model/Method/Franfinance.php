...
class Franfinance extends Systempay
{
    protected $_code = \Lyranetwork\Systempay\Helper\Data::METHOD_FRANFINANCE;
    protected $_formBlockType = \Lyranetwork\Systempay\Block\Payment\Form\Franfinance::class;
    ...
    protected function setExtraFields($order)
    {
        ...
        $this->systempayRequest->set('capture_delay', '0');

        $info = $this->getInfoInstance();

        // Override with selected FranFinance payment card.
        $option = @unserialize($info->getAdditionalInformation(\Lyranetwork\Systempay\Helper\Payment::FRANFINANCE_OPTION));
        $this->systempayRequest->set('payment_cards', $option['payment_means']);

        $fees = $option['fees'] ? 'Y' : 'N';
        $code = substr($option['payment_means'], -2);
        $this->systempayRequest->set('acquirer_transient_data', '{"FRANFINANCE":{"FEES_' . $code . '":"' . $fees . '"}}');
    }
    ...
    public function assignData(\Magento\Framework\DataObject $data)
    {
        ...
        $option = $this->getOption($systempayData->getData('systempay_franfinance_option'));
        $info->setAdditionalInformation(\Lyranetwork\Systempay\Helper\Payment::FRANFINANCE_OPTION, serialize($option));

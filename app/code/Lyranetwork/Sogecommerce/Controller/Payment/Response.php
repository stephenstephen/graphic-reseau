...
class Response extends \Magento\Framework\App\Action\Action
{
    ...
    /**
     * @var \Lyranetwork\Sogecommerce\Controller\Processor\ResponseProcessor
     */
    ...
    public function execute()
    {
        ...
        try {
            ...
            $updateOrder = $this->getRequest()->getParam('sogecommerce_update_order', false);
            if ($updateOrder) {
                return null;
            }
        ...
        if ($this->dataHelper->getCommonConfigData('ctx_mode', $storeId) === 'TEST') {
            ...
            if ($checkUrlWarn) {
                ...
                if ($this->dataHelper->isMaintenanceMode()) {
                    ...
                    $message = __('The automatic validation has not worked. Have you correctly set up the notification URL in your Sogecommerce Back Office?');
                    $message .= '&nbsp;';
    ...
    private function createResult($path, $params)
    {
        if ($this->getRequest()->getParam('iframe', false)) {
            ...
            $block = $result->getLayout()
                ->createBlock(\Lyranetwork\Sogecommerce\Block\Payment\Iframe\Response::class)
                ->setTemplate('Lyranetwork_Sogecommerce::payment/iframe/response.phtml')

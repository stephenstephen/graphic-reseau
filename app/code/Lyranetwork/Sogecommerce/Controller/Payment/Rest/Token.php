...
class Token extends \Magento\Framework\App\Action\Action
{
    ...
    /**
     * @var \Lyranetwork\Sogecommerce\Model\Method\Standard
     */
     ...
     * @param \Lyranetwork\Sogecommerce\Model\Method\Standard $standardMethod
    ...
    public function __construct(
        ...
        \Lyranetwork\Sogecommerce\Model\Method\Standard $standardMethod,
    ...
    public function execute()
    {
        ...
        $action = $this->getRequest()->getParam('sogecommerce_action');
        ...
        switch ($action) {
            ...
            case self::SET_TOKEN:
                ...
                $checkout->setData('sogecommerce_last_real_id', $lastIncrementId);
            ...
            case self::RESTORE_CART:
                $lastIncrementId = $checkout->getData('sogecommerce_last_real_id');
                if (! $lastIncrementId) {
                    return;
                }
                ...
                if ($quote->getId() && ! $quote->getIsActive() && ($this->standardMethod->getConfigData('rest_attempts') !== '0')) {
                    $checkout->setData('sogecommerce_last_real_id', null);
            ...
            case self::GET_TOKEN_AMOUNT_IN_CENTS:
                ...
                $currency = $currencyCode ? SogecommerceApi::findCurrencyByAlphaCode($currencyCode) : null;
    ...
    {
        if ($this->getRequest()->getParam('sogecommerce_action') == self::REFRESH_TOKEN) {
            $quote = $this->dataHelper->getCheckoutQuote();

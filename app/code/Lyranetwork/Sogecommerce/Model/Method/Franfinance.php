...
class Franfinance extends Sogecommerce
{
    protected $_code = \Lyranetwork\Sogecommerce\Helper\Data::METHOD_FRANFINANCE;
    protected $_formBlockType = \Lyranetwork\Sogecommerce\Block\Payment\Form\Franfinance::class;
    ...
    protected function setExtraFields($order)
    {
        ...
        $this->sogecommerceRequest->set('validation_mode', '0');
        $this->sogecommerceRequest->set('capture_delay', '0');

        $info = $this->getInfoInstance();

        // Override with selected Franfinance payment card.
        $option = @unserialize($info->getAdditionalInformation(\Lyranetwork\Sogecommerce\Helper\Payment::FRANFINANCE_OPTION));
        $this->sogecommerceRequest->set('payment_cards', $option['payment_means']);

        $fees = $option['fees'] ? 'Y' : 'N';
        $code = substr($option['payment_means'], -2);
        $this->sogecommerceRequest->set('acquirer_transient_data', '{"FRANFINANCE":{"FEES_' . $code . '":"' . $fees . '"}}');
    }
    ...
    public function assignData(\Magento\Framework\DataObject $data)
    {
        ...
        $option = $this->getOption($sogecommerceData->getData('sogecommerce_franfinance_option'));
        $info->setAdditionalInformation(\Lyranetwork\Sogecommerce\Helper\Payment::FRANFINANCE_OPTION, serialize($option));

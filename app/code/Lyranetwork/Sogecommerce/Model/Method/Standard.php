...
use Lyranetwork\Sogecommerce\Helper\Data;

class Standard extends Sogecommerce
{
    protected $_code = \Lyranetwork\Sogecommerce\Helper\Data::METHOD_STANDARD;
    protected $_formBlockType = \Lyranetwork\Sogecommerce\Block\Payment\Form\Standard::class;
    ...
    public function __construct(
        ...
        parent::__construct(
            ...
            $logger,
            $localeResolver,
            $sogecommerceRequestFactory,
            $sogecommerceResponseFactory,
            $transaction,
            $transactionResource,
            $urlBuilder,
            $redirect,
    ...
    protected function setExtraFields($order)
    {
        ...
        if ($this->isLocalCcType()) {
            // Set payment_cards.
            $this->sogecommerceRequest->set('payment_cards', $info->getCcType());
        } else {
            ...
            $this->sogecommerceRequest->set('payment_cards', $paymentCards);
        }

        // Set payment_src to MOTO for backend payments.
        if ($this->dataHelper->isBackend()) {
            $this->sogecommerceRequest->set('payment_src', 'MOTO');
            $this->sogecommerceRequest->addExtInfo('admin_user_id', $this->authSession->getUser()->getId());
            return;
        }

        if ($this->isIframeMode()) {
            // Iframe enabled.
            $this->sogecommerceRequest->set('action_mode', 'IFRAME');

            // Hide logos below payment fields.
            $this->sogecommerceRequest->set('theme_config', $this->sogecommerceRequest->get('theme_config') . '3DS_LOGOS=false;');

            // Enable automatic redirection.
            $this->sogecommerceRequest->set('redirect_enabled', '1');
            $this->sogecommerceRequest->set('redirect_success_timeout', '0');
            $this->sogecommerceRequest->set('redirect_error_timeout', '0');

            $returnUrl = $this->sogecommerceRequest->get('url_return');
            $this->sogecommerceRequest->set('url_return', $returnUrl . '?iframe=true');
        }
        ...
        if ($this->isOneClickActive() && $order->getCustomerId()) {
            ...
            if ($customer->getCustomAttribute('sogecommerce_identifier') && $this->customerSession->getValidAlias()) {
                // Customer has an identifier.
                $this->sogecommerceRequest->set('identifier', $customer->getCustomAttribute('sogecommerce_identifier')->getValue());

                if (! $info->getAdditionalInformation(\Lyranetwork\Sogecommerce\Helper\Payment::IDENTIFIER)) {
                    // Customer choose to not use alias.
                    $this->sogecommerceRequest->set('page_action', 'REGISTER_UPDATE_PAY');
                }
            } else {
                ...
                $this->sogecommerceRequest->set('page_action', 'ASK_REGISTER_PAY');
            }
        }
    ...
    public function getAvailableCcTypes()
    {
        ...
        // All cards.
        $allCards = \Lyranetwork\Sogecommerce\Model\Api\Form\Api::getSupportedCardTypes();
    ...
    public function isOneclickAvailable()
    {
        ...
        if (! $customer || ! ($identifier = $customer->getCustomAttribute('sogecommerce_identifier'))) {
            return false;
        }
    ...
    public function assignData(\Magento\Framework\DataObject $data)
    {
        ...
        $sogecommerceData = $this->extractPaymentData($data);

        $info->setCcType($sogecommerceData->getData('sogecommerce_standard_cc_type'));

        // Whether to do a payment by identifier.
        $info->setAdditionalInformation(
            \Lyranetwork\Sogecommerce\Helper\Payment::IDENTIFIER,
            $sogecommerceData->getData('sogecommerce_standard_use_identifier')
        );
    ...
    protected function getRestApiFormTokenData($quote)
    {
        ...
        $currency = \Lyranetwork\Sogecommerce\Model\Api\Form\Api::findCurrencyByAlphaCode($quote->getQuoteCurrencyCode());
        if (($this->dataHelper->getCommonConfigData('online_transactions_currency') == '2') || (($this->dataHelper->getCommonConfigData('online_transactions_currency') !== '2') && ! $currency)) {
            $currency = \Lyranetwork\Sogecommerce\Model\Api\Form\Api::findCurrencyByAlphaCode($quote->getBaseCurrencyCode());
    ...
    protected function getTokenDataForOrder($order)
    {
        ...
        $currency = \Lyranetwork\Sogecommerce\Model\Api\Form\Api::findCurrencyByAlphaCode($order->getOrderCurrencyCode());
        if (($this->dataHelper->getCommonConfigData('online_transactions_currency') == '2') || (($this->dataHelper->getCommonConfigData('online_transactions_currency') !== '2') && ! $currency)) {
            $currency = \Lyranetwork\Sogecommerce\Model\Api\Form\Api::findCurrencyByAlphaCode($order->getBaseCurrencyCode());
    ...
    public function getRestApiFormToken($renew = false)
    {
        ...
        $tokenDataName = \Lyranetwork\Sogecommerce\Helper\Payment::TOKEN_DATA;
        $tokenName = \Lyranetwork\Sogecommerce\Helper\Payment::TOKEN;
        ...
        try {
            // Perform our request.
            $client = new \Lyranetwork\Sogecommerce\Model\Api\Rest\Api(

...
class Other extends Sogecommerce
{
    protected $_code = \Lyranetwork\Sogecommerce\Helper\Data::METHOD_OTHER;
    protected $_formBlockType = \Lyranetwork\Sogecommerce\Block\Payment\Form\Other::class;
    ...
    protected function setExtraFields($order)
    {
        ...
        $option = @unserialize($info->getAdditionalInformation(\Lyranetwork\Sogecommerce\Helper\Payment::OTHER_OPTION));

        // Check if capture_delay and validation_mode are overriden.
        if (is_numeric($option['capture_delay'])) {
            $this->sogecommerceRequest->set('capture_delay', $option['capture_delay']);
        }

        if ($option['validation_mode'] !== '-1') {
            $this->sogecommerceRequest->set('validation_mode', $option['validation_mode']);
        }

        // Add cart data.
        if ($option['cart_data'] === '1') {
            $this->checkoutHelper->setCartData($order, $this->sogecommerceRequest, true);
        }

        if ($option['embedded_mode'] === '1') {
            $this->sogecommerceRequest->set('embedded_mode', $option['embedded_mode']);
        }
    }
    ...
    public function assignData(\Magento\Framework\DataObject $data)
    {
        ...
        $option = $this->_getMeans($sogecommerceData->getData('sogecommerce_other_option'));
        if ($option) {
            $info->setCcType($option['means'])->setAdditionalInformation(\Lyranetwork\Sogecommerce\Helper\Payment::OTHER_OPTION, serialize($option));
        }
    ...
    */
    ...
    {
        $supportedCards = \Lyranetwork\Sogecommerce\Model\Api\Form\Api::getSupportedCardTypes();

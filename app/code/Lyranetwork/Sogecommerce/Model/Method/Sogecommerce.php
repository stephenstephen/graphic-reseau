...
use Lyranetwork\Sogecommerce\Model\Api\Form\Api as SogecommerceApi;
use Lyranetwork\Sogecommerce\Model\Api\Rest\Api as SogecommerceRest;
use Lyranetwork\Sogecommerce\Model\Api\Refund\Api as SogecommerceRefund;
use Lyranetwork\Sogecommerce\Model\Api\Refund\OrderInfo as SogecommerceOrderInfo;

abstract class Sogecommerce extends \Magento\Payment\Model\Method\AbstractMethod
{
    const CART_MAX_NB_PRODUCTS = 85;

    protected $_infoBlockType = \Lyranetwork\Sogecommerce\Block\Payment\Info::class;
    ...
    /**
     * @var \Lyranetwork\Sogecommerce\Model\Api\Form\Request
     */
    protected $sogecommerceRequest;

    /**
     * @var \Lyranetwork\Sogecommerce\Model\Api\Form\Response
     */
    protected $sogecommerceResponse;
    ...
    /**
     * @var \Lyranetwork\Sogecommerce\Helper\Payment
     */
    protected $paymentHelper;

    /**
     * @var \Lyranetwork\Sogecommerce\Helper\Checkout
     */
    protected $checkoutHelper;

    /**
     * @var \Lyranetwork\Sogecommerce\Helper\Rest
     */
    protected $restHelper;

    /**
     * @var \Lyranetwork\Sogecommerce\Helper\Refund
     */
     ...
     * @param \Lyranetwork\Sogecommerce\Model\Api\Form\Request $sogecommerceRequest
     * @param \Lyranetwork\Sogecommerce\Model\Api\Form\ResponseFactory $sogecommerceResponseFactory
     ...
     * @param \Lyranetwork\Sogecommerce\Helper\Payment $paymentHelper
     * @param \Lyranetwork\Sogecommerce\Helper\Checkout $checkoutHelper
     * @param \Lyranetwork\Sogecommerce\Helper\Rest $restHelper
     * @param \Lyranetwork\Sogecommerce\Helper\Refund $refundHelper
    ...
    public function __construct(
        ...
        \Lyranetwork\Sogecommerce\Model\Api\Form\RequestFactory $sogecommerceRequestFactory,
        \Lyranetwork\Sogecommerce\Model\Api\Form\ResponseFactory $sogecommerceResponseFactory,
        ...
        \Lyranetwork\Sogecommerce\Helper\Payment $paymentHelper,
        \Lyranetwork\Sogecommerce\Helper\Checkout $checkoutHelper,
        \Lyranetwork\Sogecommerce\Helper\Rest $restHelper,
        \Lyranetwork\Sogecommerce\Helper\Refund $refundHelper,
        ...
        $this->sogecommerceRequest = $sogecommerceRequestFactory->create();
        $this->sogecommerceResponseFactory = $sogecommerceResponseFactory;
    ...
    {
        // Set order_id.
        $this->sogecommerceRequest->set('order_id', $order->getIncrementId());

        $online_transactions_currency = $this->dataHelper->getCommonConfigData('online_transactions_currency');
        $amount = $order->getGrandTotal();
        $currency = SogecommerceApi::findCurrencyByAlphaCode($order->getOrderCurrencyCode());
        if (($online_transactions_currency == '2') || (($online_transactions_currency !== '2') && ! $currency)) {
            // Use base currency.
            $currency = SogecommerceApi::findCurrencyByAlphaCode($order->getBaseCurrencyCode());
            $amount = $order->getBaseGrandTotal();
        }

        $this->sogecommerceRequest->set('currency', $currency->getNum());

        // Set the amount to pay.
        $this->sogecommerceRequest->set('amount', $currency->convertAmountToInteger($amount));

        // Contrib info.
        $this->sogecommerceRequest->set('contrib',  $this->dataHelper->getContribParam());
        ...
        foreach ($configFields as $field) {
            $this->sogecommerceRequest->set($field, $this->dataHelper->getCommonConfigData($field));
        }

        // Check if capture_delay and validation_mode are overriden in submodules.
        if (is_numeric($this->getConfigData('capture_delay'))) {
            $this->sogecommerceRequest->set('capture_delay', $this->getConfigData('capture_delay'));
        }

        if ($this->getConfigData('validation_mode') !== '-1') {
            $this->sogecommerceRequest->set('validation_mode', $this->getConfigData('validation_mode'));
        }
        ...
        $this->sogecommerceRequest->set('url_return', $returnUrl);

        // Set the language code.
        $this->sogecommerceRequest->set('language', $this->getPaymentLanguage());
        ...
        $this->sogecommerceRequest->set('available_languages', $availableLanguages);
        ...
        $this->sogecommerceRequest->set('threeds_mpi', $threedsMpi);

        $this->sogecommerceRequest->set('cust_email', $order->getCustomerEmail());
        $this->sogecommerceRequest->set('cust_id', $order->getCustomerId());
        $this->sogecommerceRequest->set('cust_title', $order->getBillingAddress()->getPrefix() ?
            $order->getBillingAddress()->getPrefix() : null);
        $this->sogecommerceRequest->set('cust_first_name', $order->getBillingAddress()->getFirstname());
        $this->sogecommerceRequest->set('cust_last_name', $order->getBillingAddress()->getLastname());
        $this->sogecommerceRequest->set('cust_address', implode(' ', $order->getBillingAddress()->getStreet()));
        $this->sogecommerceRequest->set('cust_zip', $order->getBillingAddress()->getPostcode());
        $this->sogecommerceRequest->set('cust_city', $order->getBillingAddress()->getCity());
        $this->sogecommerceRequest->set('cust_state', $order->getBillingAddress()->getRegionCode());
        $this->sogecommerceRequest->set('cust_country', $order->getBillingAddress()->getCountryId());
        $this->sogecommerceRequest->set('cust_phone', $telephone);
        $this->sogecommerceRequest->set('cust_cell_phone', $telephone);

        $address = $order->getShippingAddress();
        if (is_object($address)) { // Shipping is supported.
            $this->sogecommerceRequest->set('ship_to_first_name', $address->getFirstname());
            $this->sogecommerceRequest->set('ship_to_last_name', $address->getLastname());
            $this->sogecommerceRequest->set('ship_to_city', $address->getCity());
            $this->sogecommerceRequest->set('ship_to_street', $address->getStreetLine(1));
            $this->sogecommerceRequest->set('ship_to_street2', $address->getStreetLine(2));
            $this->sogecommerceRequest->set('ship_to_state', $address->getRegionCode());
            $this->sogecommerceRequest->set('ship_to_country', $address->getCountryId());
            $this->sogecommerceRequest->set('ship_to_phone_num', str_replace([' ', '.', '-'], '', $address->getTelephone() ? $address->getTelephone() : ''));
            $this->sogecommerceRequest->set('ship_to_zip', $address->getPostcode());
        }

        $features = \Lyranetwork\Sogecommerce\Helper\Data::$pluginFeatures;
        if ($features['brazil']) {
            $this->sogecommerceRequest->set('cust_national_id', $this->getCustomerData($order, 'cpf'));
            $this->sogecommerceRequest->set('cust_address_number', $this->getCustomerData($order, 'street'));
            $this->sogecommerceRequest->set('cust_district', $this->getCustomerData($order, 'district'));

            if (is_object($address)) {
                $this->sogecommerceRequest->set('ship_to_user_info', $this->getCustomerData($order, 'cpf'), false);
                $this->sogecommerceRequest->set('ship_to_address_number', $this->getCustomerData($order, 'street'), false);
                $this->sogecommerceRequest->set('ship_to_district', $this->getCustomerData($order, 'district'), false);
            }
        }
        ...
        if ($sendCartDetails || $this->needsCartData /* Cart data are mandatory for the payment method. */) {
            $this->checkoutHelper->setCartData($order, $this->sogecommerceRequest);
        }
        ...
        if ($this->needsShippingMethodData /* Shipping method data are mandatory for the payment method. */) {
            $this->checkoutHelper->setAdditionalShippingData($order, $this->sogecommerceRequest);
        }

        $paramsToLog = $this->sogecommerceRequest->getRequestFieldsArray(true);
        $this->dataHelper->log('Payment parameters: ' . json_encode($paramsToLog));

        return $this->sogecommerceRequest->getRequestFieldsArray(false, false);
    }
    ...
    public function getPaymentLanguage()
    {
        ...
        if (! SogecommerceApi::isSupportedLanguage($lang)) {
    ...
    public function resetData()
    {
        ...
        $keys = [
            \Lyranetwork\Sogecommerce\Helper\Payment::MULTI_OPTION,
            \Lyranetwork\Sogecommerce\Helper\Payment::OTHER_OPTION,
            \Lyranetwork\Sogecommerce\Helper\Payment::CHOOZEO_OPTION,
            \Lyranetwork\Sogecommerce\Helper\Payment::FULLCB_OPTION,
            \Lyranetwork\Sogecommerce\Helper\Payment::ONEY_OPTION,
            \Lyranetwork\Sogecommerce\Helper\Payment::IDENTIFIER,
            \Lyranetwork\Sogecommerce\Helper\Payment::SEPA_IDENTIFIER
        ];
    ...
    public function acceptPayment(\Magento\Payment\Model\InfoInterface $payment)
    {
        ...
        try {
            ...
            $uuid = $payment->getAdditionalInformation(\Lyranetwork\Sogecommerce\Helper\Payment::TRANS_UUID);
            ...
            $successStatuses = array_merge(SogecommerceApi::getSuccessStatuses(), SogecommerceApi::getPendingStatuses());
            ...
            // Load API response.
            $response = $this->sogecommerceResponseFactory->create(
                [
                    'params' => $data,
                    'ctx_mode' => null,
                    'key_test' => '',
            ...
            if ($e->getCode() === 'PSP_100') {
                ...
                if ($e->getCode() <= -1) {
                    // Manage cUrl errors.
                    $message .= __('Please consult the Sogecommerce logs for more details.');
                } else {
    ...
    public function denyPayment(\Magento\Payment\Model\InfoInterface $payment)
    {
        ...
        if (! $this->restHelper->getPrivateKey($storeId)) {
            ...
            $this->messageManager->addWarningMessage(__('Payment is cancelled only in Magento. Please, consider cancelling the payment in Sogecommerce Back Office.'));
            return true;
        }
    ...
    {
        return $this->sogecommerceValidatePayment($payment);
    }

    protected function sogecommerceValidatePayment($payment, $createInvoice = true)
    {
        // Clear all messages in session.
        $this->messageManager->getMessages(true);
        ...
        try {
            ...
            $option = @unserialize($payment->getAdditionalInformation(\Lyranetwork\Sogecommerce\Helper\Payment::MULTI_OPTION));
            $multi = (stripos($payment->getMethod(), 'sogecommerce_multi') === 0) && is_array($option) && ! empty($option);
            $count = $multi ? (int) $option['count'] : 1;

            // Retrieve saved transaction UUID.
            $savedUuid = $payment->getAdditionalInformation(\Lyranetwork\Sogecommerce\Helper\Payment::TRANS_UUID);
    ...
    {
        $this->messageManager->addWarningMessage(__('Payment is validated only in Magento. Please, consider validating the payment in Sogecommerce Back Office.'));
    ...
    public function canUseForCurrency($baseCurrencyCode)
    {
        ...
        if ($quote && $quote->getId()) {
            ...
            $currency = SogecommerceApi::findCurrencyByAlphaCode($currencyCode);
            if ($currency) {
                return true;
            }
        }
        ...
        $currency = SogecommerceApi::findCurrencyByAlphaCode($baseCurrencyCode);
        if ($currency) {
            return true;
        }
     ...
     * @param \Magento\Payment\Model\InfoInterface $payment
     * @param float $amount
     * @return Lyranetwork\Sogecommerce\Model\Method\Sogecommerce
     */
    public function refund(\Magento\Payment\Model\InfoInterface $payment, $amount)
    {
        ...
        try {
            ...
            $sogecommerceOrderInfo->setOrderRemoteId($order->getIncrementId());
            $sogecommerceOrderInfo->setOrderId($order->getIncrementId());
            $sogecommerceOrderInfo->setOrderReference($order->getIncrementId());
            $sogecommerceOrderInfo->setOrderCurrencyIsoCode($currencyCode);
            $sogecommerceOrderInfo->setOrderCurrencySign($currencyCode);
            $sogecommerceOrderInfo->setOrderUserInfo($commentText);
            ...
            $refundApi->refund($sogecommerceOrderInfo, $amountInCurrencyCode);
        } catch (\Exception $e) {
            throw new \Exception($e->getMessage());
        }
    ...
    public function capture(\Magento\Payment\Model\InfoInterface $payment, $amount)
    {
        ...
        return $this->sogecommerceValidatePayment($payment, false);
    }

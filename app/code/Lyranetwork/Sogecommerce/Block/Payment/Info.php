 ...
 */
namespace Lyranetwork\Sogecommerce\Block\Payment;

use Lyranetwork\Sogecommerce\Model\Api\Form\Response as SogecommerceResponse;
...
class Info extends \Magento\Payment\Block\Info
{
     ...
     * @var string
     */
    protected $_template = 'Lyranetwork_Sogecommerce::payment/info.phtml';
    ...
    /**
     * @var \Lyranetwork\Sogecommerce\Block\Adminhtml\System\Config\Form\Field\ContactSupport
     */
     ...
     * @param \Lyranetwork\Sogecommerce\Block\Adminhtml\System\Config\Form\Field\ContactSupport $supportBlock
     * @param array $data
     */
    public function __construct(
        ...
        \Lyranetwork\Sogecommerce\Block\Adminhtml\System\Config\Form\Field\ContactSupport $supportBlock,
        array $data = []
    ) {
    ...
    public function getResultDescHtml()
    {
        $allResults = @unserialize(
            $this->getInfo()->getAdditionalInformation(\Lyranetwork\Sogecommerce\Helper\Payment::ALL_RESULTS)
        );
        ...
        if (! is_array($allResults) || empty($allResults)) {
            ...
            $restError = $this->getInfo()->getAdditionalInformation(\Lyranetwork\Sogecommerce\Helper\Payment::REST_ERROR_MESSAGE);
            if ($restError) {
                $labels[] = $restError;
                unset($keys[0]);
            }
            ...
            foreach ($keys as $key) {
                ...
                if (($key === 'result') && ((int)$allResults[$key] === 30)) { // Append form error if any.
                    $label .= ' ' . SogecommerceResponse::extraMessage($allResults['extra_result']);
                }
    ...
    public function getPaymentDetailsHtml($backend = true)
    {
        ...
        if ($backend) {
            $userChoice = $payment->getAdditionalInformation(\Lyranetwork\Sogecommerce\Helper\Payment::BRAND_USER_CHOICE);
            if ($userChoice === true) {
    ...
    public function getMultiPaymentDetailsHtml($backend = true)
    {
        ...
        $userChoice = $this->getInfo()->getAdditionalInformation(\Lyranetwork\Sogecommerce\Helper\Payment::BRAND_USER_CHOICE);
    ...
    public function translate($code, $type, $appendCode = false)
    {
        ...
        return SogecommerceResponse::translate($code, $type, $lang, $appendCode);
    }

...
define(
    ...
    function(
        ...
        return Component.extend({
            defaults: {
                template: 'Lyranetwork_Sogecommerce/payment/sogecommerce-standard',
                sogecommerceCcType:  window.checkoutConfig.payment.sogecommerce_standard.availableCcTypes ?
                    window.checkoutConfig.payment.sogecommerce_standard.availableCcTypes[0]['value'] : null,
            ...
            getData: function() {
                ...
                if (this.isOneClick()) {
                    data['additional_data']['sogecommerce_standard_use_identifier'] = this.sogecommerceUseIdentifier();
                }

                if (this.getEntryMode() == 2) { // Payment means selection on merchant site.
                    data['additional_data']['sogecommerce_standard_cc_type'] = this.sogecommerceCcType();
                }
            ...
            getRestFormToken: function() {
                return window.checkoutConfig.payment.sogecommerce_standard.restFormToken || null;
            },
            ...
            getLanguage: function() {
                return window.checkoutConfig.payment.sogecommerce_standard.language || null;
            },

            updatePaymentBlock: function(blockName) {
                $('.payment-method._active .payment-method-content .sogecommerce-identifier li.sogecommerce-block').hide();
                $('li.sogecommerce-standard-' + blockName + '-block').show();
            },

            updateOrder: function() {
                return (window.checkoutConfig.payment.sogecommerce_standard.updateOrder
                    && (window.checkoutConfig.payment.sogecommerce_standard.updateOrder == '1'))
                    || false;
            },
            ...
            restoreCart: function() {
                if (!PLACE_ORDER) {
                    storage.post(
                        url.build('sogecommerce/payment_rest/token?sogecommerce_action=restore_cart&form_key=' + $.mage.cookies.get('form_key'))
            ...
            manageError: function (e) {
                ...
                $('#sogecommerce_rest_form .kr-form-error').html('<span style="color: red;"><span>' + msg + '</span></span>');
                return true;
            },
            ...
            checkPayload: function() {
                ...
                if (!quote.paymentMethod() || !quote.paymentMethod().hasOwnProperty('method') || quote.paymentMethod().method !== 'sogecommerce_standard') {
                    ...
                    KR.setFormConfig({
                        ...
                        function(v) {
                            ...
                            KR.onFocus(function(e) {
                                $('#sogecommerce_rest_form .kr-form-error').html('');
                            });
            ...
            refreshToken: function() {
                ...
                storage.post(
                    url.build('sogecommerce/payment_rest/token?sogecommerce_action=refresh_token&form_key=' + $.mage.cookies.get('form_key'))
                ).done(function(response) {
                    if (response.token) {
                        if (me.getCompactMode() === "1") {
                            KR.setFormConfig({ cardForm: { layout: "compact" }, smartForm: { layout: "compact" }});
            ...
            setToken: function() {
                ...
                storage.post(
                    url.build('sogecommerce/payment_rest/token?sogecommerce_action=set_token&form_key=' + $.mage.cookies.get('form_key'))
                ).done(function(response) {
                    if (response.token) {
                        if (me.getCompactMode() === "1") {
                            KR.setFormConfig({ cardForm: { layout: "compact" }, smartForm: { layout: "compact" }});
                            ...
                            function(v) {
                                ...
                                KR.onError(function(e) {
                                    ...
                                    if (answer && (answer.clientAnswer.orderStatus == "UNPAID") && (answer.clientAnswer.orderCycle == "CLOSED")) {
                                        ...
                                        data['sogecommerce_update_order'] = true;
            ...
            checkTokenAmount: function(displayAmount, displayCurrency, baseAmount, baseCurrency, krAmount) {
                ...
                storage.post(
                    url.build('sogecommerce/payment_rest/token?sogecommerce_action=get_token_amount_in_cents' + '&displayAmount=' + displayAmount + '&displayCurrency=' + displayCurrency + '&baseAmount=' + baseAmount + '&baseCurrency=' + baseCurrency + '&form_key=' + $.mage.cookies.get('form_key'))
                ).done(function(response) {
            ...
            initIframeEvents: function(elts) { // To be called after iframe loading.
                ...
                quote.paymentMethod.subscribe(function(method) {
                    if ((method.method === 'sogecommerce_standard') || !me.iframeDisplayed) {
                        return;
                    }

                    var iframe = $('.payment-method .payment-method-content iframe.sogecommerce-iframe');
                    if (iframe && iframe.length) {
                        var cancelUrl = me.getIframeLoaderUrl() + '?mode=cancel' + '&' + Date.now();
                        iframe.attr('src', cancelUrl);

                        $('.payment-method .payment-method-content .sogecommerce-iframe').hide();
                        $('.payment-method .payment-method-content .sogecommerce-form').show();
            ...
            afterPlaceOrder: function() {
                ...
                if (((me.getEntryMode() == 4) || (me.getEntryMode() == 5) || (me.getEntryMode() == 6) || (me.getEntryMode() == 7)) && me.getRestFormToken()) {
                    ...
                    $('.payment-method._active .payment-method-content .sogecommerce-form').hide();
                    $('.payment-method._active .payment-method-content .sogecommerce-iframe').show();

                    var iframe = $('.payment-method._active .payment-method-content iframe.sogecommerce-iframe');
                    if (iframe && iframe.length) {

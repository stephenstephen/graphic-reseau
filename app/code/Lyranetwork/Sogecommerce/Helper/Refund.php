...
use Lyranetwork\Sogecommerce\Model\Api\Refund\WsException as WsException;

class Refund implements \Lyranetwork\Sogecommerce\Model\Api\Refund\Processor
{
    /**
    ...
    public function doOnError($errorCode, $message)
    {
        if ($errorCode === 'PSP_100') {
            ...
            $notice = __('You are not authorized to do this action online. Please, do not forget to update payment in Sogecommerce Back Office.');
                ...
                if (! empty($leftOutTransactionsAmount)) {
                    if ($transRefundAmount == $totalLeftOutAmount) {
                        ...
                        if (($trsUuid = array_search($transRefundAmount, $leftOutTransactionsAmount)) !== false) {
                            ...
                            throw new \Exception(__('The requested refund amount does not correspond to any refund transaction in the Sogecommerce Back Office.'));
                        }
                    }
                } else {
                    ...
                    throw new \Exception(__('Refund of split payment is already done in Sogecommerce Back Office. Please, consider making an offline refund in Magento.'));
                }
            } else {
                // Order already up to date, it's a new request of refund.
                throw new \Exception(sprintf(__('Refund of split payment is not supported. Please, consider making necessary changes in %1$s Back Office.'), 'Sogecommerce'));
            }
        }
    }
    ...
    private function createRefundTransaction($payment, $refundResponse)
    {
        ...
        // Save paid amount.
        $currency = SogecommerceApi::findCurrencyByNumCode($response['vads_currency']);
        ...
        if (isset($response['vads_effective_currency']) &&
            ...
            $effectiveCurrency = SogecommerceApi::findCurrencyByNumCode($response['vads_effective_currency']);

 ...
 */
namespace Lyranetwork\Sogecommerce\Helper;
...
class Payment
{
    ...
    const IDENTIFIER = 'sogecommerce_identifier';
    const SEPA_IDENTIFIER = 'sogecommerce_sepa_identifier';

    const TOKEN_DATA = 'sogecommerce_token_data'; // Key to save payment token data.
    const TOKEN = 'sogecommerce_token'; // Key to save payment token.

    // Key to save choosen multi option.
    const MULTI_OPTION = 'sogecommerce_multi_option';

    // Key to save choosen Choozeo option.
    const CHOOZEO_OPTION = 'sogecommerce_choozeo_option';

    // Key to save choosen Oney option.
    const ONEY_OPTION = 'sogecommerce_oney_option';

    // Key to save choosen Full CB option.
    const FULLCB_OPTION = 'sogecommerce_fullcb_option';

    // Key to save choosen Franfinance option.
    const FRANFINANCE_OPTION = 'sogecommerce_franfinance_option';

    // Key to save choosen other payment means.
    const OTHER_OPTION = 'sogecommerce_other_option';

    // Key to save risk control results.
    const RISK_CONTROL = 'sogecommerce_risk_control';

    // Key to save risk assessment results.
    const RISK_ASSESSMENT = 'sogecommerce_risk_assessment';

    // Key to save payment results.
    const ALL_RESULTS = 'sogecommerce_all_results';

    // Key to save Rest Api error message.
    const REST_ERROR_MESSAGE = 'sogecommerce_rest_error';

    const TRANS_UUID = 'sogecommerce_trans_uuid';

    const BRAND_USER_CHOICE = 'sogecommerce_brand_user_choice';
     ...
     * @param \Lyranetwork\Sogecommerce\Model\Api\Form\Response $response
     */
    public function registerOrder(
        \Magento\Sales\Model\Order $order,
        \Lyranetwork\Sogecommerce\Model\Api\Form\Response $response
    ) {
        ...
        if ($method instanceof \Lyranetwork\Sogecommerce\Model\Method\Sepa)  {
    ...
    public function nextOrderState(
        ...
        if ($response->isToValidatePayment()) {
            $newStatus = 'sogecommerce_to_validate';
            ...
            if ($this->isSepa($response)) {
                ...
                $newStatus = 'sogecommerce_pending_transfer';
            } else {
    ...
    public function updatePaymentInfo(\Magento\Sales\Model\Order $order, \Lyranetwork\Sogecommerce\Model\Api\Form\Response $response)
    {
        ...
        $order->getPayment()
            ...
            ->setAdditionalInformation(\Lyranetwork\Sogecommerce\Helper\Payment::ALL_RESULTS, serialize($response->getAllResults()))
            ->setAdditionalInformation(\Lyranetwork\Sogecommerce\Helper\Payment::TRANS_UUID, $response->get('trans_uuid'));
        ...
        if ($restErrorMsg) {
            ...
            $order->getPayment()->setAdditionalInformation(\Lyranetwork\Sogecommerce\Helper\Payment::REST_ERROR_MESSAGE, $restErrorMsg);
        } else {
            $order->getPayment()->unsAdditionalInformation(\Lyranetwork\Sogecommerce\Helper\Payment::REST_ERROR_MESSAGE);
        }

        $currency = SogecommerceApi::findCurrencyByNumCode($response->get('currency'));
        ...
        if ($response->get('brand_management')) {
            ...
            $order->getPayment()->setAdditionalInformation(\Lyranetwork\Sogecommerce\Helper\Payment::BRAND_USER_CHOICE, $userChoice);
        }
        ...
        if ($response->get('card_brand') === 'MULTI') { // Multi brand.
            ...
            $option = @unserialize($order->getPayment()->getAdditionalInformation(\Lyranetwork\Sogecommerce\Helper\Payment::MULTI_OPTION));
            ...
            if ($isFirstInstallment && (stripos($order->getPayment()->getMethod(), 'sogecommerce_multi') === 0)
                ...
                for ($i = 1; $i <= $count; $i++) {
                    ...
                    if (($rate = $response->get('change_rate')) && $response->get('effective_currency') &&
                        ...
                        // Effective amount.
                        $effectiveCurrency = SogecommerceApi::findCurrencyByNumCode($response->get('effective_currency'));
    ...
    public function saveIdentifier(
        ...
        if ($response->get('identifier') && (
            ...
            $customerData->setCustomAttribute('sogecommerce_identifier', $response->get('identifier'));

            // Mask card number and save it to customer entity.
            $customerData->setCustomAttribute('sogecommerce_masked_pan', $this->maskPan($response));

            try {
                $customer->updateData($customerData);

                $customerResource = $this->customerResourceFactory->create();
                $customerResource->saveAttribute($customer, 'sogecommerce_identifier');
                $customerResource->saveAttribute($customer, 'sogecommerce_masked_pan');
    ...
    public function saveSepaIdentifier(
        ...
        if ($response->get('identifier') && (
            ...
            $customerData->setCustomAttribute('sogecommerce_sepa_identifier', $response->get('identifier'));

            // Mask IBAN and save it to customer entity.
            $customerData->setCustomAttribute('sogecommerce_sepa_iban_bic', $this->maskPan($response));

            try {
                $customer->updateData($customerData);

                $customerResource = $this->customerResourceFactory->create();
                $customerResource->saveAttribute($customer, 'sogecommerce_sepa_identifier');
                $customerResource->saveAttribute($customer, 'sogecommerce_sepa_iban_bic');
     ...
     *
     * @param string $sogecommerceType
     * @return string
     */
    public function convertTransactionType($sogecommerceType)
    {
        $type = false;

        switch ($sogecommerceType) {

 ...
 */
namespace Lyranetwork\Sogecommerce\Helper\Payment;
...
class Data extends \Magento\Payment\Helper\Data
{
    ...
    public function getPaymentMethods()
    {
        ...
        $sogecommerceMultiTitle = $methods['sogecommerce_multi']['title']; // Get multi payment general title.
        unset($methods['sogecommerce_multi']);
        ...
        foreach ($this->dataHelper->getMultiPaymentModelConfig() as $config) {
            ...
            $count = substr($code, strlen('sogecommerce_multi_'));

            $methods[$code] = [
                'model' => $config['value'],
                'title' => "$sogecommerceMultiTitle ($count)",
                'group' => 'sogecommerce'
            ];
        }

        $sogecommerceOtherTitle = $methods['sogecommerce_other']['title']; // Get other payment means general title.
        unset($methods['sogecommerce_other']);

        // Add other payment virtual methods to the list.
        foreach ($this->dataHelper->getOtherPaymentModelConfig() as $config) {
            $code = substr($config['path'], strlen('payment/'), - strlen('/model'));
            $means = substr($code, strlen('sogecommerce_other_'));

            $methods[$code] = [
                'model' => $config['value'],
                'title' => $sogecommerceOtherTitle . " ($means)",
                'group' => 'sogecommerce'
            ];
        }
    ...
    {
        if (strpos($code, 'sogecommerce_other_') === 0) {
            return parent::getMethodInstance('sogecommerce_other');
        }

...
{
    const METHOD_STANDARD = 'sogecommerce_standard';
    const METHOD_MULTI = 'sogecommerce_multi';
    const METHOD_CHOOZEO = 'sogecommerce_choozeo';
    const METHOD_SEPA = 'sogecommerce_sepa';
    const METHOD_GIFT = 'sogecommerce_gift';
    const METHOD_ONEY = 'sogecommerce_oney';
    const METHOD_PAYPAL = 'sogecommerce_paypal';
    const METHOD_FULLCB = 'sogecommerce_fullcb';
    const METHOD_FRANFINANCE = 'sogecommerce_franfinance';
    const METHOD_OTHER = 'sogecommerce_other';
    ...
    /**
     * @var \Lyranetwork\Sogecommerce\Model\Logger\Sogecommerce
     */
    protected $logger;
    ...
    /**
     * @param \Lyranetwork\Sogecommerce\Helper\Context $context
     * @param \Magento\Store\Model\StoreManagerInterface $storeManager
     * @param \Magento\Framework\App\MaintenanceMode $maintenanceMode
     * @param \Magento\Config\Model\ResourceModel\Config $resourceConfig
     * @param \Magento\Framework\Filesystem $filesystem
     * @param \Lyranetwork\Sogecommerce\Model\Logger\Sogecommerce
    ...
    public function __construct(
        \Lyranetwork\Sogecommerce\Helper\Context $context,
        \Magento\Store\Model\StoreManagerInterface $storeManager,
        \Magento\Framework\App\MaintenanceMode $maintenanceMode,
        \Magento\Config\Model\ResourceModel\Config $resourceConfig,
        \Magento\Framework\Filesystem $filesystem,
        \Lyranetwork\Sogecommerce\Model\Logger\Sogecommerce $logger,
    ...
    public function getCommonConfigData($field, $storeId = null)
    {
        ...
        $value = $this->scopeConfig->getValue('sogecommerce/general/' . $field, $scope, $storeId);
    ...
    public function getReturnUrl($storeId = null)
    {
        ...
        return $this->_getUrl('sogecommerce/payment/response', $params);
    }

    public function getRestReturnUrl()
    {
        return $this->_getUrl('sogecommerce/payment_rest/response', ['_secure' => true]);
    }
    ...
    {
        $filePath = $this->filesystem->getDirectoryRead(DirectoryList::MEDIA)->getAbsolutePath('sogecommerce/images/' . $fileName);
    ...
    {
        // $path is as payment_[lang]/sogecommerce/sogecommerce_[group]/sogecommerce_[sub_group]
        $parts = explode('/', $path);
    ...
    public function updateMultiPaymentModelConfig($options)
    {
        foreach ($options as $option) {
            $this->resourceConfig->saveConfig(
                'payment/sogecommerce_multi_' . $option['count'] . 'x/model',
                \Lyranetwork\Sogecommerce\Model\Method\Multix::class,
    ...
    public function getMultiPaymentModelConfig()
    {
        ...
        $select = $connection->select()
            ...
            ->where('path LIKE ?', 'payment/sogecommerce\_multi\_%x/model');
    ...
    public function updateOtherPaymentModelConfig($options)
    {
        foreach ($options as $option) {
            $this->resourceConfig->saveConfig(
                'payment/sogecommerce_other_' . $option['means'] . '/model',
                \Lyranetwork\Sogecommerce\Model\Method\Other::class,
                ScopeConfigInterface::SCOPE_TYPE_DEFAULT,
                0
            );
        }
    ...
    public function getOtherPaymentModelConfig()
    {
        ...
        $select = $connection->select()
            ->from($this->resourceConfig->getMainTable())
            ->where('path LIKE ?', 'payment/sogecommerce\_other\_%/model');
    ...
    public function getLogoImageSrc($name)
    {
        if ($this->isUploadFileImageExists($name)) {
            return $this->storeManager->getStore()->getBaseUrl(\Magento\Framework\UrlInterface::URL_TYPE_MEDIA) .
                'sogecommerce/images/' . $name;
        } else {
            // Default logo from the installed plugin.
            $asset = $this->assetRepo->createAsset('Lyranetwork_Sogecommerce::images/' . $name);

            if ($this->isPublishFileImageExists($asset->getRelativeSourceFilePath())) {
                return $this->getViewFileUrl('Lyranetwork_Sogecommerce::images/' . $name);
            }
        }
    ...
    {
        $standardMethod = $this->getMethodInstance(\Lyranetwork\Sogecommerce\Helper\Data::METHOD_STANDARD);
        $sepaMethod = $this->getMethodInstance(\Lyranetwork\Sogecommerce\Helper\Data::METHOD_SEPA);
    ...
    public function getCardDataEntryMode($storeId = null)
    {
        ...
        $value = $this->scopeConfig->getValue('payment/sogecommerce_standard/card_info_mode', $scope, $storeId);
    ...
    public function getContribParam() {
        ...
        return $cmsParam . '/' . $cmsVersion . '/' . SogecommerceApi::shortPhpVersion();
    }

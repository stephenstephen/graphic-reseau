    ...
    */
    ...
    public function checkCustormers($scope, $scopeId)
    {
        ...
        foreach ($collection as $customer) {
            if (! preg_match(self::CUST_ID_REGEX, $customer->getId())) {
                ...
                $msg = '';
                $msg .= __(
                    'Customer ID &laquo; %1 &raquo; does not match Sogecommerce specifications.',
                    $customer->getId()
                )->render() . ' ';
    ...
    private function checkOrderId($orderId)
    {
        if (! preg_match(self::ORDER_ID_REGEX, $orderId)) {
            ...
            $msg = '';
            $msg .= __(
                'The next order ID  &laquo; %1 &raquo; does not match Sogecommerce specifications.',
                $orderId
            )->render() . ' ';
    ...
    public function checkProducts($scope, $scopeId)
    {
        ...
        foreach ($collection as $product) {
            if (! preg_match(self::PRODUCT_REF_REGEX, $product->getId())) {
                ...
                $msg = '';
                $msg .= __(
                    'Product reference &laquo; %1 &raquo; does not match Sogecommerce specifications.',
                    $product->getId()
                )->render() . ' ';
    ...
    public function toSogecommerceCarrier($methodCode)
    {
    ...
    public function toSogecommerceCategory($categoryIds)
    {
        ...
        if (is_array($categoryMapping) && ! empty($categoryMapping) && is_array($categoryIds) && ! empty($categoryIds)) {
            foreach ($categoryMapping as $category) {
                if (in_array($category['code'], $categoryIds)) {
                    return $category['sogecommerce_category'];
                }
            }
        }
    ...
    public function setCartData($order, &$sogecommerceRequest)
    {
        $notAllowed = '#[^A-Z0-9ÁÀÂÄÉÈÊËÍÌÎÏÓÒÔÖÚÙÛÜÇ ]#ui';

        // Used currency.
        $currency = SogecommerceApi::findCurrencyByNumCode($sogecommerceRequest->get('currency'));
        ...
        foreach ($order->getAllItems() as $item) {
            ...
            if (! $item->getParentItem()) {
                ...
                $sogecommerceRequest->addProduct(
                    preg_replace($notAllowed, ' ', $label),
                    $priceInCents,
                    $qty,
                    $item->getProductId(),
                    $this->toSogecommerceCategory($categoryIds)
                );
        ...
        $sogecommerceRequest->set('insurance_amount', 0); // By default, shipping insurance amount is not available in Magento.
        $shippingAmount = $isDisplayCurrency ? $order->getShippingAmount() : $order->getBaseShippingAmount();
        $sogecommerceRequest->set('shipping_amount', $currency->convertAmountToInteger($shippingAmount));

        // Recalculate tax_amount to avoid rounding problems.
        $taxAmount = $sogecommerceRequest->get('amount') - $subtotal - $sogecommerceRequest->get('shipping_amount') -
             $sogecommerceRequest->get('insurance_amount');
        ...
        $sogecommerceRequest->set('tax_amount', $taxAmount);

        // VAT amount for colombian payment means.
        $sogecommerceRequest->set('totalamount_vat', $taxAmount);
    }

    public function setAdditionalShippingData($order, &$sogecommerceRequest)
    {
        // By default, Magento customers are private.
        $sogecommerceRequest->set('cust_status', 'PRIVATE');
        $sogecommerceRequest->set('ship_to_status', 'PRIVATE');

        // If this is Oney.
        $oneyMeans = ['ONEY_3X_4X', 'ONEY_10X_12X', 'ONEY_PAYLATER'];
        $useOney = in_array($sogecommerceRequest->get('payment_cards'), $oneyMeans);
        ...
        if ($order->getIsVirtual() || ! $order->getShippingMethod()) { // There is no shipping method.
            $sogecommerceRequest->set('ship_to_type', 'ETICKET');
            $sogecommerceRequest->set('ship_to_speed', 'EXPRESS');
        } else {
            $shippingMethod = $this->toSogecommerceCarrier($order->getShippingMethod());
            ...
            switch ($shippingMethod['type']) {
                ...
                case 'RECLAIM_IN_SHOP':
                    ...
                    $sogecommerceRequest->set('ship_to_street', $address);
                    $sogecommerceRequest->set('ship_to_street2', $name);
                    ...
                    $sogecommerceRequest->set('ship_to_delivery_company_name', preg_replace($notAllowedCharsRegex, ' ', $name));
                ...
                default:
                    if ($useOney) {
                        ...
                        $sogecommerceRequest->set('ship_to_street', $address);
                        $sogecommerceRequest->set('ship_to_street2', null); // Not sent to FacilyPay Oney.
                    }

                    $sogecommerceRequest->set('ship_to_delivery_company_name', preg_replace($notAllowedCharsRegex, ' ', $carrierName));

                    break;
            }

            $sogecommerceRequest->set('ship_to_type', empty($shippingMethod['type']) ? null : $shippingMethod['type']);
            $sogecommerceRequest->set('ship_to_speed', empty($shippingMethod['speed']) ? null : $shippingMethod['speed']);

            if ($shippingMethod['speed'] === 'PRIORITY') {
                $sogecommerceRequest->set('ship_to_delay', empty($shippingMethod['delay']) ? null : $shippingMethod['delay']);
            }

            if ($useOney) { // Modify address to be sent to Oney.
                // TODO update parameters sent to Oney.
                $sogecommerceRequest->set('cust_country', 'FR'); // Send FR even address is in DOM-TOM unless form is rejected.

                $sogecommerceRequest->set('ship_to_state', null); // Not sent to Oney.
                $sogecommerceRequest->set('ship_to_country', 'FR'); // Send FR even address is in DOM-TOM unless form is rejected.
            }
        }
    }
    ...
    {
        if ($this->cookieManager->getCookie('sogecommerce_oney_error')) {

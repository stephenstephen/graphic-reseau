...
class MauticCheckoutOrders implements \Magento\Framework\Event\ObserverInterface
{
    /**
     ...
     */
    private $checkExportCustomers;

    /**
     * @var \Magento\Framework\App\Config\ScopeConfigInterface
     */
    private $checkScopeConfig;
    ...
    public function __construct(
        ...
        $this->checkConnectionManager = $connectionManager;
        $this->checkExportCustomers = $exportCustomers;
        $this->checkScopeConfig = $scopeConfig;
    }
    ...
    {
        $exportType = $this->checkScopeConfig->getValue('mautic_integration/mautic_customer_export/export_type');
        if ($this->checkConnectionManager->getMauticConfig('enable') == 1 &&
            ...
            if ($customerId) {
                try {
                    $this->checkExportCustomers->exportCustomerOrder($customerId);
                } catch (\Exception $e) {
                    $this->checkConnectionManager->createLog('Exception in Checkout orders:- ' .
                        $e->getMessage(), '', '', '');
                }
            }
        }

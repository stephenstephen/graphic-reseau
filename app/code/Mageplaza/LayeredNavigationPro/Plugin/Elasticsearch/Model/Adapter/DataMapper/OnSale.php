...
class OnSale implements DataMapperInterface
{
    ...
    public function map($entityId, array $entityIndexData, $storeId, $context = [])
    {
        ...
        foreach ($customerGroupCollection as $customerGroup) {
            $mapperData[self::FIELD_NAME . '_' . $customerGroup->getId() . '_' . $websiteId] = (int)$this->isProductOnSale($entityId, $storeId, $customerGroup->getId());;
        }
    ...
    {
        $this->scopeConfig->isSetFlag('layered_navigation/filter/state/onsales_enable', ScopeInterface::SCOPE_STORE);
    }
